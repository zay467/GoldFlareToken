...
var GoldFlareTokenSale = artifacts.require("GoldFlareTokenSale");

contract("GoldFlareTokenSale", function (accounts) {
  var tokenInstance;
  ...
  it("initialize the contract with correct values", function () {
    return GoldFlareTokenSale.deployed()
    ...
    return GoldFlareToken.deployed()
      ...
      .then(function (balance) {
        assert.equal(
          balance.toNumber(),
          999990,
          "return all unsold token to admin"
        );
        return tokenSaleInstance.tokenPrice();
      })
      .then(assert.fail)
      .catch(function (error) {
        assert("selfdestructed");
      });
  });
});
