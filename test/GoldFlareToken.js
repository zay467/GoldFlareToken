var GoldFlareToken = artifacts.require("GoldFlareToken");

contract("GoldFlareToken", function (accounts) {
  var tokenInstance;
  it("initialize the contract with the correct values", function () {
    return GoldFlareToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
        return tokenInstance.name();
      })
      .then(function (name) {
        assert.equal(name, "GoldFlare", "has the correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "Lit", "has the correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "GoldFlare V-1.0", "has the correct standard");
      });
  });
